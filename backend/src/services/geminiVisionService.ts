...
export class GeminiVisionService {
  private static getApiKey(customApiKey?: string): string {
    ...
    if (!key) {
      const error: any = new Error(
        'API Key is missing. Please configure a key in the server configuration or specify it in your request settings.'
      );
   ...
   */
  public static async generateVisionResponse(params: GenerateVisionParams) {
    const {
      message,
      imageBuffer,
      mimeType,
    ...
    try {
      ...
      const apiError: any = new Error(
        error.message || 'Error communicating with Gemini Vision API.'
      );
